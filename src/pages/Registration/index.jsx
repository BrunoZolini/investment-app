...
export default function Registration() {
  ...
  const validateEmail = () => {
    ...
    if (!emailRegex.test(email)) {
      // verifica se o email tem o formato esperado
  ...
  const validateName = () => {
    const TWO = 2;
    if (name.length <= TWO) {
      // verifica se o name é maior que 3
  ...
  const validatePassword = () => {
    ...
    if (password.length <= FIVE) {
      // verifica se a senha é maior que 5
  ...
  const validateConfirmPassword = () => {
    if (password !== confirmPassword) {
      // confirma se o password esta igual a confirmação de password
